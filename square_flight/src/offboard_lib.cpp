...
MultiDOFControl::MultiDOFControl(const ros::NodeHandle &nh, const ros::NodeHandle &nh_private, bool manual_input): nh_(nh),
    ...
    nh_private_.getParam("/offb_node/desired_planning_velocity", vel_planning_desired_);
...
void MultiDOFControl::setMultiDOFPoints() {
    for(int i = 0; i < 10; i ++){
        ...
        vel_.linear.x = vel_planning_desired_;
        vel_.linear.y = vel_planning_desired_;
        vel_.linear.z = vel_planning_desired_;
        //set acceleration
        acc_.angular.x = 0;
        acc_.angular.y = 0;
        acc_.angular.z = 0;
...
void MultiDOFControl::multiDOFFlight() {
    ...
    while(ros::ok() ) {
        ...
        pos_target_.position.x = opt_points[0].transforms[0].translation.x;
        pos_target_.position.y = opt_points[0].transforms[0].translation.y;
        pos_target_.position.z = opt_points[0].transforms[0].translation.z;
...
void MultiDOFControl::velTest() {
    ros::Rate rate(10);
    pos_target_.position.x = 0;
    pos_target_.position.y = 0;
    pos_target_.position.z = 10;
    // pos_target_.velocity.x = 100;
    // pos_target_.velocity.y = 100;
    pos_target_.velocity.z = 100;
    while(ros::ok()){
        mavros_pub_.publish(pos_target_);
        rate.sleep();
    }
